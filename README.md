...
Download Ethereum Studio install-package in [Github Release](https://github.com/ObsidianLabs/EthereumStudio/releases) according to the computer system type (`.dmg` for macOS, `.AppImage` for Linux, `.exe` for Windows).
...
Users can create, import and manage keypairs in the keypair manager. Click *Create* button and follow the prompts to key in a name and create a keypair. When creating or editing a keypair, users can set an alias for the keypair to facilitate identification in later use. Ethereum Studio creates new keypair with a private key by default but mnemonic is also supported. Simply click on the arrow next to *Regenerate* button and select from the drop-down list.
...
Before we proceed, please create a few keypairs in the Keypair Manager. We'll be using 3 keypairs in the following demos, namely  `miner` , `my-keypair-1`  , `my-keypair-2` . 
...
Block Explorer will display all holdings of ERC-20 token that are accepted by [Trust Wallet](https://github.com/trustwallet/assets/blob/master/blockchains/ethereum/tokenlist.json) at the address when connecting to mainnet.
...
Right now, we'll create a new project named `my-project` with Basics template from Open Zeppelin and Truffle framework. Click *Create Project* to start the process. 
